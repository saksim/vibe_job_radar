...
2026-09-24 后续实测（Issue #152）发现一个已返回的独立职位介绍使用“工作职能 / 任职资格”，原字面检查在该条报 `structure_changed`。仅将这两个已观察标记加入上述结构化与语义介绍检查；不会把单独“资格”或宣传文字当作正文，也不放宽身份、JSON-LD 一致性、隐藏、截断、推荐隔离检查。公开回归使用独立编写的人工文字，真实存档只在本机；旧失败仍保留，离线存档重解析与新的实时尝试分别记录，不回写原失败来增加成功率。

同日第30项在职责中出现“公司信息部”，被原“公司信息”子串误当成独立公司资料。仅排除已观察的完整词组“公司信息部”；“公司信息”附栏及其他推荐标记继续拒绝。两种介绍结构均覆盖正反例，原失败另记，未通过改写历史结果提高成功率。
