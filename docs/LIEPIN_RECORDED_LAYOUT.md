...
2026-09-24 的算法分类第26项另暴露一种误拒绝：可见职位介绍与同职位结构化全文一致，但直接列出连续编号的学历/技能条件，没有“岗位要求”等小标题。本分支按 #160 增加有界识别：必须有唯一可见 `dd`，全文与同身份的结构化正文完全一致，且有3～20条从1开始连续编号、每条以已观察的明确资格词开头的条件。无可见佐证、仅结构化前缀、缺号、普通营销编号列表仍不接受；身份、截断、公司/推荐内容和长度检查保持。原失败、只读旧响应校验与独立新尝试分别记录，不改写旧采集结果，也不改变目标岗位匹配规则。

## 2026-09-26：分节职责和技能没有通用小标题

第三日期预登记的公开架构师分类前3项得到2份正文，第2项报 `structure_changed`，见 #54 / #208。保存响应有唯一可见职位介绍、同岗位唯一 JobPosting、规范化后一致的1139字全文；没有截断、隐藏或公司/推荐混入。该岗位直接分节说明工作、再列技能条件，没有原检查认可的“职责/要求”等字面标记，也不是连续编号条件。

新增识别仅用于完整可见介绍与同身份结构化全文相等的情况：正文5～100行，至少3个不同工作分节标签，标签含设计/实现/产品化/建设/管理/探索等明确工作词，节内须有工作动作及至少20字内容；同时至少2条不同的明确技能/经验条件，每条至少20字。一般宣传、重复标签、缺少资格条件、结构化前缀或未见可见介绍仍拒绝。身份、隐藏、展开/登录、推荐隔离、字数和旧编号条件检查保持。

6项新增合成回归覆盖完整保真与上述边界。保存响应只读重放已保留全部1139字、0新请求/0新报告；原实采 **2成功/1失败**、第三日原报告、原89条和第二日报告不变。重放不是新一次实站成功，也不更改登录/网络契约或认证等级。
