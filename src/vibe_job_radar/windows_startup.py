...
def command_line(executable, workspace):
    ...
    result = quoted(executable) + ' --workspace ' + quoted(workspace)
...
class WindowsStartup:
    ...
    def _receipt(self):
        ...
        try:
            ...
            if (not isinstance(value, dict) or set(value) != _FIELDS
                    ...
                    or value['command'] != command_line(value['executable'], value['workspace'])):
    ...
    def _inspect(self):
        base = dict(supported=self.supported, can_enable=False, can_disable=False,
                    ...
                    executable=str(self.executable), value_name=self.name)
        ...
        if actual is None:
            ...
            state = {**base, 'status': 'registered' if same else 'moved',
                     ...
                     'message': ('已登记登录启动。Windows 可能延迟或禁用启动，请同时核对系统的启动应用设置。'
    ...
    def state(self):
        try:
            ...
            return dict(supported=self.supported, can_enable=False, can_disable=False,
                        ...
                        workspace=str(self.workspace), executable=str(self.executable), value_name=self.name,
    ...
    def _change(self, data, *, enable):
        ...
        if enable and (data['consent'] is not True or data['consent_version'] != CONSENT):
            raise InputError('请明确同意登录 Windows 后打开当前程序和工作区。')
        if not self.supported:
