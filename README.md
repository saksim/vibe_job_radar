...
本分支按#122提供[28项改进统一候选](docs/CURRENT_DELIVERY_CANDIDATE.md)，包含原25项、[独立公开计划进程](docs/PUBLIC_WORKER.md)、[Cursor公开目录](docs/PUBLIC_SOURCES.md)及[登录后明确选择独立进程](docs/WINDOWS_LOGIN_STARTUP.md)，供运行和审查，尚未合入main或正式发布。源码使用`codex/delivery-28`，Windows候选使用同一提交的CI产物；当前验证与未完成目标分别记录，不需自行拼接功能分支。
...
本分支按#107修复[英文要求与产品提及边界](docs/ENGLISH_OBLIGATION_BOUNDARIES.md)：公司/福利介绍不进入要求统计，明确正负条款分开，含糊工具提及保留待复核。规则版本升为`rules-0.2.0`；旧报告保持原样，新分析需重新核对原文和个人映射。
...
登录态、简历、本人证据和私人报告默认留本地，生产 Linux 部署后置。已合并的匿名本机 HTTP/SOCKS5 静态策略和明确同意的 198.18/15 加密解析保留；本分支新增的[显式本机代理认证](docs/LOCAL_PROXY_AUTH.md)由#75跟踪。#109新增默认关闭的[Windows本地PAC导入](docs/WINDOWS_DOMAIN_PAC.md)，按域名选择明确路线，失败不推进至直连。高级批次接线及其他网络缺口见 [网络状态](docs/NETWORK_COMPATIBILITY_STATUS.md)。不要关闭 TLS 校验或删除配额库来制造成功。
