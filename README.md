...
本分支按#192补充[回归失败的源码位置](docs/TEST_FAILURE_LOCATION.md)：长断言不再遮住失败代码行，原线程栈、结果与时限保持；该诊断补充不代表历史等待根因已解决。
...
#181继续补充[明确借助语言模型审查代码](docs/MODEL_CODE_REVIEW.md)的使用语境，保留产品开发、普通模型知识和不同条款的边界。每次重算独立保存原文与规则配置，当前提交的验证结果以对应PR为准。
...
新报告会在[各方向样本覆盖](docs/ROLE_SAMPLE_COVERAGE.md)中分开显示完整正文、含AI编程证据的岗位数和待复核要求，便于判断哪些方向还缺样本。旧报告未记录的计数保持未知。
...
想先取得实际正文，可从首页主动选择“进入猎聘架构师公开分类采集”或“进入猎聘算法工程师公开分类采集”，核对用途和每批1～5项后确认。该路径已有完整正文与报告证据，无需账号或搜索 Key，不使用自定义关键词/地区，详见[按按钮取数指南](docs/GUIDED_COLLECTION.md)。它是可选入口，不切换默认或在失败后自动回退。

需要关键词搜索或正常登录时，使用浏览器向导：检查采集浏览器，选择平台、关键词、1页/5条和实际访问范围，点击“打开搜索并读取岗位”，按需在程序弹出的独立浏览器正常登录，再读取列表、勾选采集。猎聘原生实验模式已有本机 Edge 单页关键词列表观察；默认浏览器桥、正常账号到完整JD及三站闭环仍待实站验收，按钮流程不代表已验通。

程序负责汇总列表观察到的链接、打开详情、保存最终地址和独立正文、生成本批报告。你不必填写 API endpoint、分页游标、选择器，也不必逐条复制详情 URL。没有稳定链接或结构不兼容的页面需要逐站维护，不会编造结果。

**猎聘新增一次正常密码表单提交，当前通过受控测试，尚未完成真实账号验收。** 第 3 步可在本机明确授权填写并提交一次，账号密码不会写入任务、报告或会话文件；协议、验证码和短信验证由你在平台页面完成，原查询或所选完整岗位可读后自动接续。其他平台继续使用人工登录。真实账号与所选全文接续仍待验证，详见 [密码登录范围](docs/LIEPIN_PASSWORD_LOGIN.md)及[当前实测记录](docs/CURRENT_ACQUISITION_EVIDENCE.md)。默认退出服务不保存 Cookie；第 2 步明确勾选“在此工作区本机保留”后可尝试恢复本站 Cookie，再通过正常页面继续取数。Windows 使用当前用户 DPAPI，Linux/macOS 使用仅所有者权限且不加密；不保存密码、localStorage 或 IndexedDB，不保证账号仍有效。使用与清除方法见 [本机会话恢复](docs/SAVED_SESSION_D04.md)。三站均为 `not_live_verified`，没有真实账号与样本验收不能宣称实站成功。
...
分类页因网络错误停止、尚未取得名单时，可[显式保存同页重试](docs/CATEGORY_PAGE_RETRY.md)：核对原链接、页号与最多5条范围后保存暂停任务，原失败和配额保留。每页最多三次显式重试，网络恢复后才由用户继续。
...
浏览器向导和高级页公开分类的当前批次由后端线程执行，关工作台网页仍继续；应用须保持运行，暂停/停止控制任务，退出后须明确继续。其他高级 HTTP 路线仍由网页驱动后续步骤。固定公开目录另有可选的[每日计划](docs/PUBLIC_DAILY_SCHEDULE.md)及[Windows便携包登录启动](docs/WINDOWS_LOGIN_STARTUP.md)，不自动登录三站或安排分类定时采集，不保证无人登录全天运行。
...
当前#63候选针对已复现的控制台事件采集影响，改用[按需页面控制](docs/NATIVE_EVENT_COLLECTION.md)。保留浏览器自动化标识、原脚本、请求校验与空白失败处理；后续已取得本机单页关键词列表，真实账号与所选完整JD仍待验收。

同一候选接入[正常搜索框](docs/LIEPIN_NORMAL_SEARCH_FORM.md)：默认关键词从正常入口填写并提交一次，核对当前查询响应后才提取职位。地区目录及输入建议只允许已核对的精确读取，实际含参文档仍遵守robots；当前单页观察不认证其他筛选条件、账号或跨日期闭环。
