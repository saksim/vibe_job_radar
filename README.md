...
研究架构师或算法方向时，可直接点击首页的**进入猎聘架构师公开分类采集**或**进入猎聘算法工程师公开分类采集**：无需账号或搜索 Key，核对用途和范围后自动取得一批正文，结束后可继续同一名单的下一批。算法是宽分类，报告继续按原垂直领域算法和时间序列规则筛选，不匹配的完整正文也会保存。这是固定公开分类，每批1～5项，不执行首页所填关键词或地区检索；打开入口不会自动联网或勾选确认。范围见[公开分类说明](docs/LIEPIN_PUBLIC_CATEGORY.md)。
