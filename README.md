...
本分支按#188提供[52项改进统一候选](docs/CURRENT_DELIVERY_CANDIDATE.md)：在原47项上纳入明确AI应用、超时诊断、模型代码审查、分类页同页恢复与robots前导通配符修复。当前89条完整职位记录中49个符合原目标规则，报告含94条要求（24规则接受、70待复核），实际采集日期为2026-09-24。源码使用`codex/delivery-52`；正常关键词搜索、真实账号会话和多站正文仍有待验项，候选尚未合入main或发布。Windows包须使用本提交自己的成功CI产物。
...
需要关键词搜索或正常登录时，使用浏览器向导：检查采集浏览器，选择平台、关键词、1页/5条和实际访问范围，点击“打开搜索并读取岗位”，按需在程序弹出的独立浏览器正常登录，再读取列表、勾选采集。当前猎聘关键词入口仍会清空，三站浏览器闭环尚未实站认证，按钮流程不代表已验通。
...
同一候选接入[正常搜索框](docs/LIEPIN_NORMAL_SEARCH_FORM.md)：默认关键词从正常入口填写并提交一次，核对当前查询响应后才提取职位。地区目录及输入建议只允许已核对的精确读取，实际含参文档仍遵守robots；真实表单参数和账号尚待验通。
